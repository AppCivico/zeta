...
no strict 'refs';
*{"main::$_"} = *$_ for grep { defined &{$_} } keys %Zeta::Test::Further::;
use strict 'refs';
...
sub api_auth_as {
    ...
    if ( !$auth_user ) {
        use Package::Stash;
        use Zeta::TestOnly::Mock::AuthUser;

        my $stashc = Package::Stash->new('Catalyst::Plugin::Authentication');
        $auth_user = Zeta::TestOnly::Mock::AuthUser->new;

        $stashc->add_symbol( '&user',  sub { return $auth_user } );
        $stashc->add_symbol( '&_user', sub { return $auth_user } );
    }

    $Zeta::TestOnly::Mock::AuthUser::_id    = $conf{user_id};
    @Zeta::TestOnly::Mock::AuthUser::_roles = @{ $conf{roles} };
}

sub db_transaction (&) {
    my ($subref) = @_;

    my $schema = Zeta->model('DB');
